...
def dv_parse_document(session: Session, document: PolicyDocument) -> PolicyDocument:
    ...
    try:
        ...
        parsed_path = _dv_write_parsed_text(document.id, segments)
...
def _dv_write_parsed_text(document_id: str, segments: list[ParsedSegment]) -> Path:
    dv_settings.dv_parsed_dir.mkdir(parents=True, exist_ok=True)
    parsed_path = dv_settings.dv_parsed_dir / f"{document_id}.txt"
