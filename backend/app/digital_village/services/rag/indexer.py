...
from app.digital_village.services.rag.llama_index_adapter import (
    build_dv_policy_node,
    dv_delete_document_nodes,
    dv_index_nodes,
)


def dv_rebuild_rag_index(session: Session) -> dict[str, int]:
    ...
    for document in documents:
        indexed = dv_index_document(session, document)
        document_count += 1
        chunk_count += indexed
...
def dv_index_document(session: Session, document: PolicyDocument) -> int:
    ...
    dv_delete_document_nodes(str(document.id))
    ...
    for draft in drafts:
        metadata = _build_dv_chunk_metadata(document, attachment)
        metadata.update(
            {
                "source": "dv_llamaindex_indexer",
    ...
    session.flush()
    nodes = [build_dv_policy_node(chunk, document, attachment) for chunk in indexed_chunks]
    dv_index_nodes(nodes)

    document.parse_status = "indexed"
    session.commit()
    _dv_refresh_search_vectors(session, str(document.id))
    session.refresh(document)
    return len(indexed_chunks)


def _build_dv_chunk_metadata(
...
def _dv_refresh_search_vectors(session: Session, document_id: str) -> None:
    session.execute(
        text(
            """
