...
from app.digital_village.services.rag.indexer import dv_rebuild_rag_index, dv_index_document
...
router = APIRouter(tags=["digital-village-rag"])


@router.post("/rag/index/rebuild", response_model=RAGIndexResponse)
def rebuild_index(
    db: Annotated[Session, Depends(get_digital_village_db)],
) -> RAGIndexResponse:
    result = dv_rebuild_rag_index(db)
    return RAGIndexResponse(**result)


@router.post("/rag/index/documents/{document_id}", response_model=RAGIndexResponse)
def rebuild_document_index(
    ...
    chunk_count = dv_index_document(db, document)
...
def _run_search(db: Session, request: RAGSearchRequest) -> RAGSearchResponse:
    ...
    results = dv_hybrid_search(db, request.query, filters, request.top_k)
