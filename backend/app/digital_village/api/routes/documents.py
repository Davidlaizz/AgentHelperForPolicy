...
router = APIRouter(tags=["digital-village-documents"])
...
async def upload_document(
    ...
    if auto_parse:
        document = dv_parse_document(db, document)

    return serialize_document(db, document)


@router.post("/documents/{document_id}/parse", response_model=PolicyDocumentResponse)
...
@router.patch("/documents/{document_id}", response_model=PolicyDocumentResponse)
def update_document(
    document_id: str,
...
@router.delete("/documents/{document_id}", response_model=PolicyDocumentResponse)
...
async def save_upload_file(file: UploadFile, file_type: str) -> tuple[Path, int, str]:
    ...
    target_path = dv_settings.dv_upload_dir / f"{document_id}.{file_type}"
    ...
    with target_path.open("wb") as output:
        while chunk := await file.read(1024 * 1024):
            ...
            if total_size > dv_settings.max_upload_file_size_bytes:
                output.close()
                target_path.unlink(missing_ok=True)
                raise HTTPException(
                    status_code=413,
                    detail=f"文件大小不能超过 {dv_settings.dv_max_upload_file_size_mb}MB",
                )
            digest.update(chunk)
            output.write(chunk)
