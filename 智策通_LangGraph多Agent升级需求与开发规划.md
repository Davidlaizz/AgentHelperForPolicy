...
1. 将当前卡片式 Agent 图升级为更直观的流程图。（已完成 P2.1：单张 SVG 编排图，并优化为“主链路 + 证据中枢 + 缺口追问 + 判断行动治理 + 生成与记忆 + ReAct 虚线闭环”布局）
2. 可选技术：第一阶段 Mermaid 静态图，第二阶段 React Flow 交互图。（已采用轻量前端交互图，暂不引入 React Flow）
3. 增加单次运行详情弹窗或独立页面。（已完成 P2.2：节点/边详情面板和运行观测中心）
...
1. `/admin` 已展示多 Agent 状态图、条件路由和运行观测中心，支持在同一区域查看最近运行与单次运行时间线。
2. 节点卡片和节点治理统计表已展示调用次数、平均耗时、失败次数、输入字段、输出字段。
3. 后端管理接口已返回 `call_count`、`last_failure_message`、`last_failure_at` 等治理字段。
4. 编排图已在同一张图中展示 Agent 节点、边、方向和条件，节点采用中文职责名与英文 Agent 名两行展示，支持点击节点查看入参、出参和最近运行摘要，并支持按钮缩放和鼠标滚轮缩放。
5. 边已支持点击查看条件、条件表达式和中文业务解释。
6. 已新增 ReAct 虚线反馈闭环：追问后用户补充重新进入槽位抽取，记忆写入后下一轮重新进入记忆读取。
7. `/admin` 浏览器验证已确认编排图渲染为 `1320x620` SVG 画布，默认缩放为 `60%`。
8. `npm run lint` 和 `npm run build` 均通过。
