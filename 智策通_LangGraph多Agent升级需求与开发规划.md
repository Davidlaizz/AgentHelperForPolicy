...
1. 将当前卡片式 Agent 图升级为更直观的流程图。（已完成 P2.1：单张 SVG 编排图）
2. 可选技术：第一阶段 Mermaid 静态图，第二阶段 React Flow 交互图。（已采用轻量前端交互图，暂不引入 React Flow）
3. 增加单次运行详情弹窗或独立页面。（已完成 P2.2：节点/边详情面板和运行详情面板）
...
4. 编排图已在同一张图中展示 Agent 节点、边、方向和条件，支持点击节点查看入参、出参和最近运行摘要。
5. 边已支持点击查看条件、条件表达式和中文业务解释。
6. `npm run lint` 和 `npm run build` 均通过。
